defmodule MayaWeb.RssController do
  ...
  defp get_entry(conn, %{title: title, slug: slug, description: description, ahash: ahash, extension: extension, inserted_at: inserted_at}) do
    Entry.new(Routes.page_url(conn, :show_image, slug), DateTime.from_naive!(inserted_at, "Etc/UTC"), title)
    |> Entry.link(Routes.page_url(conn, :show_image, slug))
    |> Entry.author(@author)
    |> Entry.content("<img src=\"https://d2f33fmhbh7cs9.cloudfront.net/image/" <> ahash <> "/960w/" <> slug <> extension <> "\"/><p>#{description}</p>", type: "html")
    |> Entry.build()
  end
end
